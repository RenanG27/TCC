...
<html lang="pt-br">
	...
	<ul>
		...
		?>
		<form method="POST" action="pro_delete_cursos.php">
		
		<input type="hidden" name="cu_cod" value="<?php echo 
		$row_cursos['cu_cod'];?>">
		<input type="hidden" name="cu_nome" value="<?php echo 
		$row_cursos['cu_nome'];?>">
		
		<p>Codigo: <?php echo $row_cursos['cu_cod']; ?></p>
		<p>Nome: <?php echo $row_cursos['cu_nome']; ?></p>
		<h3> Essa operação é irreversivel - Deseja realmente excluir este Curso?</h3>
		<div class="botao">
			<input type="submit" class="btn btn-outline-success" value="Sim">

		<a href='index_cursos.php?pagina=1'>
